...
//     super.initState();
//     _startTimer();
//     // Add clipboard paste listener to first text field
//     _focusNodes[0].addListener(() {
//       if (_focusNodes[0].hasFocus) {
//         Clipboard.getData('text/plain').then((value) {
//           if (value?.text != null && value!.text!.length >= 4) {
//             _handlePaste(value.text!);
//           }
//         });
//       }
//     });
...
//         Get.snackbar(
//           'Error',
//           'Please enter a valid otp: ${response.statusCode}.',
//           backgroundColor: Colors.redAccent,
//           colorText: Colors.white,
//         );
//       }
...
class _VerificationPageState extends State<VerificationPage> {
  ...
  int remainingTime = 180;
  late Timer _timer;
  late String currentContact;

  @override
  void initState() {
    super.initState();
    currentContact = widget.contactInfo; // Track the contact info
    _startTimer();
    //_setupPasteListener();
  }
  ...
  Future<void> _verifyOtp() async {
    ...
    // Ensure the contact info hasn't changed
    if (currentContact != widget.contactInfo) {
      Get.snackbar(
        'Error',
        'The contact information has changed. Please request a new OTP.',
        backgroundColor: Colors.redAccent,
        colorText: Colors.white,
      );
      return;
  ...
  Widget build(BuildContext context) {
    if (currentContact != widget.contactInfo) {
      // If contact info changes, clear fields and reset the state
      _clearFields();
      currentContact = widget.contactInfo;
    }
